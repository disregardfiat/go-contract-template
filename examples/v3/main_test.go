...
func TestV3_Init_Mint_Swap_Fees_Burn_Collect(t *testing.T) {
	...
	lower := sqrtP - 1
	upper := sqrtP + 1
	Init(s("hbd,hive,30," + strconv.FormatUint(sqrtP, 10) + "," + strconv.FormatUint(lower, 10) + "," + strconv.FormatUint(upper, 10)))
	...
	Mint(s(strconv.FormatUint(lower, 10) + "," + strconv.FormatUint(upper, 10) + ",10000000,10000000"))
	if L := getU(KeyLiquidity); L == 0 {
	...
	startFG0 := getU(KeyFeeGrowth0)
	setU(KeyFeeGrowth0, startFG0+deltaFG)

	// accrue owed by updating position snapshot and then collect
	sdk.ShimSetSender(sdk.Address("hive:lp"))
	updatePositionOwed(sdk.Address("hive:lp"), lower, upper)
	owed0 := getU(posKey(sdk.Address("hive:lp"), lower, upper, "owed0"))
	...
	curL := getU(posKey(sdk.Address("hive:lp"), lower, upper, "liquidity"))
	Burn(s(strconv.FormatUint(lower, 10) + "," + strconv.FormatUint(upper, 10) + "," + strconv.FormatUint(curL/4, 10)))
	if getU(posKey(sdk.Address("hive:lp"), lower, upper, "liquidity")) != curL-curL/4 {
	...
	Collect(s(strconv.FormatUint(lower, 10) + "," + strconv.FormatUint(upper, 10)))
	if getU(posKey(sdk.Address("hive:lp"), lower, upper, "owed0")) != 0 {
		t.Fatal("owed0 not cleared after collect")
	}
	if getU(posKey(sdk.Address("hive:lp"), lower, upper, "owed1")) != 0 {
...
func TestV3_System_Gov_And_Bounds(t *testing.T) {
	...
	lower := uint64(1 << (Q - 1))
	upper := uint64(1 << (Q + 1))
	Init(s("hbd,hive,10," + strconv.FormatUint(sqrtP, 10) + "," + strconv.FormatUint(lower, 10) + "," + strconv.FormatUint(upper, 10)))
	...
	SetFee(s("50"))
	if getU(KeyFeeBps) != 50 {
		t.Fatal("fee not set by system")
	}
	// set active range with price inside
	SetActiveRange(s(strconv.FormatUint(lower/2, 10) + "," + strconv.FormatUint(upper, 10)))
	if getU(KeyActiveLower) != lower/2 {
